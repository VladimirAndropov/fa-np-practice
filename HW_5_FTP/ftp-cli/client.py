...
# Проверяем существование папки для логов, если нет - создаем
...
class Client:
    ...
    def port_validation(self, port_str: str) -> bool:
        try:
            port = int(port_str)
            return 0 < port <= 65535
        except ValueError:
    ...
    def ip_validation(self, ip_str: str) -> bool:
        try:
...
def input_thread(client):
    """Поток для обработки ввода команд пользователем"""
    while True:
...
def port_validation(port_str: str) -> bool:
    try:
        port = int(port_str)
        return 0 < port <= 65535
    except ValueError:
...
def ip_validation(ip_str: str) -> bool:
    try:
        ipaddress.ip_address(ip_str)
        return True
    except ValueError:
...
def main():
    ...
    input_thread = threading.Thread(target=input_thread, args=(client,))
    input_thread.start()

    # Главный поток остается для чтения ответов от сервера
    while True:
        response = client.receive_message()
        if response == "exit":
            print("Сервер закрыл соединение. Программа завершена.")
            break
